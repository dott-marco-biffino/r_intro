...
Un linguaggio e un ambiente per il calcolo statistico e la rappresentazione grafica dei dati.

È un progetto GNU simile al linguaggio S sviluppato da Chambers e coll. presso i Bell Labs e ne rappresenta una sua diversa implementazione con una buona retro compatibilità, nonostante alcune importanti differenze.
...
- È un linguaggio matriciale, cioè un linguaggio che:

  - Non necessita di dichiarazioni specifiche per definire vettori, matrici e array (cfr. @fig-postit), 
  
  - Gli array possono essere definiti per concatenazione, 
  - Non necessita di iterazioni per operare sugli array

- A tipizzazione debole  (cfr. @fig-postit),
...
![Un mio appunto del 2009 sul confronto fra **R** e **FORTRAN 77** relativo alla creazione di una matrice. ](figures/post-it.png){#fig-postit}
...
Diversamente da molti altri linguaggio la costante di Nepero, base dei logaritmi naturali, non è definita come costante in **R**.
...
In realtà questa è solo una scorciatoia per una chiamata di funzione più estesa
...
Ovviamente sono disponibili i comuni operatori matematici applicabili a agli operandi come il vettore ```x```
...
In realtà l'elevamento a potenza ```^``` è un operatore matematico mentre ```sqrt()``` è una funzione

I principali operatori matematici in R sono ```+```, ```-```, ```*```, ```/```, ```^```, ```%%```, ```%/%```

Per i dettagli si visiti la pagina di aiuto mediante il comando ```?Arithmetic``` 
...
Oppure arrangiare gli elementi di un vettore in una matrice
```{r matrix1}
#| echo: true
...
Utilizzare ```?c``` e ```?matrix``` per consultare le relative pagine di aiuto

Alcuni calcoli statistici e funzioni applicate al vettore ```y```:
